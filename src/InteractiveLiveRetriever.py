...
class InteractiveLiveRetriever(tk.Tk):
    ...
    def __init__(self):
        ...
        # Column 1
        tk.Label(input_frame, text="Symbol").grid(row=0, column=0, sticky='w', padx=5, pady=2)
        ...
        tk.Label(input_frame, text="Time Delta (Seconds)").grid(row=1, column=0, sticky='w', padx=5, pady=2)
        ...
        tk.Label(input_frame, text="Tick Size").grid(row=2, column=0, sticky='w', padx=5, pady=2)
        ...
        # Column 2
        tk.Label(input_frame, text="Num. Time Steps").grid(row=0, column=2, sticky='w', padx=5, pady=2)
        self.num_time_steps_entry = tk.Entry(input_frame, width=12)
        self.num_time_steps_entry.insert(0, "30")  # Default value
        self.num_time_steps_entry.grid(row=0, column=3, sticky='w', padx=5, pady=2)
        
        tk.Label(input_frame, text="Num. Levels").grid(row=1, column=2, sticky='w', padx=5, pady=2)
        ...
        button_frame.grid(row=2, column=2, columnspan=2, sticky='w', padx=5, pady=5)
        
        # Start button
        self.start_button = tk.Button(button_frame, text="Start", command=self.start_data_retrieval)
        self.start_button.pack(side=tk.LEFT, padx=5)
        
        # Stop button
        self.stop_button = tk.Button(button_frame, text="Stop", command=self.stop_data_retrieval, state=tk.DISABLED)
    ...
    def start_data_retrieval(self):
        ...
        try:
            ...
            thread = threading.Thread(target=self.data_processing_loop, args=(symbol, time_delta))
            thread.daemon = True  # Thread will close when main program exits
            thread.start()
            
        except ValueError:
            self.status_label.config(text="Please enter a valid time delta (integer)")
                ...
                try:
                    ...
                    for i in range(time_steps, 0, -1):
                        ...
                        if isinstance(next(iter(bids.keys()), "0"), str):
                            bid_prices = {float(price): volume for price, volume in bids.items()}
                            ask_prices = {float(price): volume for price, volume in asks.items()}
                        else:
                            bid_prices = bids
                            ask_prices = asks
                        
                        for j in range(2 * num_levels):
                            level = round((min_level + j * tick_size) / tick_size) * tick_size
                    ...
                    self.after(0, lambda b=bid_data, a=ask_data, l=levels: self.update_visualization(b, a, l, symbol))
    ...
    def update_visualization(self, bid_data, ask_data, levels, symbol):
        ...
        try:
            ...
            green_cmap = LinearSegmentedColormap.from_list('white_to_green', ['white', 'green'])
            ...
            sns.heatmap(bid_data.T, cmap=green_cmap, ax=self.ax, cbar=False, vmin=0)
            
            # Plot asks with red colormap on top of bids - set vmin=0 to ensure zero values are white
            sns.heatmap(ask_data.T, cmap=red_cmap, ax=self.ax, cbar=False, alpha=0.5, vmin=0)
