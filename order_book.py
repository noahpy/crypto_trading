...
def load_ob_data(
    ...
    if exchange == "bybit":
        ...
        while curr_date <= end_date:
            ...
            filepath_ob = f"{folder}ob/{curr_date.year}-{zero_pad(curr_date.month)}-{zero_pad(curr_date.day)}_{contract}_ob500.data"
            filepath_trades = f"{folder}trades/{contract}{curr_date.year}-{zero_pad(curr_date.month)}-{zero_pad(curr_date.day)}.csv.gz"
...
def build_bucket_data_set(bucket_snapshots, horizon, step_size, window_len):
    ...
    for i in range(0, len(bucket_snapshots) - horizon, step_size):
        ...
        mid_price_change = (bucket_snapshots[i]["midprice"] - bucket_snapshots[i+horizon]["midprice"])/bucket_snapshots[i+horizon]["midprice"]
...
def build_bucket_change_data_set(bucket_snapshots, horizon, step_size, window_len):
    ...
    for i in range(step_size, len(bucket_snapshots) - horizon, step_size):
        ...
        bid_change = (curr_bid_buckets - np.array(bid_buckets))/curr_bid_buckets
        ask_change = (curr_ask_buckets - np.array(ask_buckets))/curr_ask_buckets
...
def build_level_data_set(snapshots, horizon, step_size):
    ...
    bid_levels, bid_sizes = map(list, zip(*sorted(snapshots[0]["bids"].items())))
    ask_levels, ask_sizes = map(list, zip(*sorted(snapshots[0]["asks"].items())))
    ...
    for i in range(step_size, len(snapshots) - horizon, step_size):
        ...
        diff = (snapshots[i]["midprice"] - snapshots[i+horizon]["midprice"])/snapshots[i+horizon]["midprice"]
        
        curr_bid_levels, curr_bid_sizes = map(list, zip(*sorted(snapshots[i]["bids"].items())))
        curr_ask_levels, curr_ask_sizes = map(list, zip(*sorted(snapshots[i]["asks"].items())))
        
        bid_levels_change = (curr_bid_levels - np.array(bid_levels))/curr_bid_levels
        bid_sizes_change = (curr_bid_sizes - np.array(bid_sizes))/curr_bid_sizes
        
        ask_levels_change = (curr_ask_levels - np.array(ask_levels))/curr_ask_levels
        ask_sizes_change = (curr_ask_sizes - np.array(ask_sizes))/curr_ask_sizes
        
        
        # First interleave bid changes
        bid_changes = np.ravel([bid_levels_change, bid_sizes_change], order='F')

        # Then interleave ask changes
        ask_changes = np.ravel([ask_levels_change, ask_sizes_change], order='F')
